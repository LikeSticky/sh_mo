            ...
            action: function(ele, getIndex) {
                ...
                /* $("." + targetA + realIndex).load("../../guide/html/convention_" + targetB + realIndex + ".html?" + Date.now(), function(responseTxt, statusTxt, xhr) {
                    if (statusTxt == "error...F5") {
                        ...
                        var frt_tbl_tr = function frt_tbl_tr(frt_tbl) {
                        ...
                        function mobileFrameSetting(targetTbl, tdNum) {
                            console.log("11111111111111111111");
                            var targetTable = $(targetTbl);
                            var linker = targetTable.find('table tr');
                            var tdNum = tdNum - 1; //0부터 시작
    
                            if ($("html").hasClass("type-smart")) {
                                $('html').find('.mobile-frame').remove();
                            } else {
                                var htmlFrame = ''
                                htmlFrame += '<div class="mobile-frame">'
                                htmlFrame += '<iframe src="" frameborder="0" id="mFrame"></iframe>'
                                htmlFrame += '<span class="frame_close">닫기</span>'
                                htmlFrame += '</div>'
                                targetTable.last().after(htmlFrame);
                                $url = targetTable.first().find('tr:first-child td').eq(tdNum).find('a').attr('href'); //초기에 맨 위의 링크가 나오도록 함
                                frameChange($url); //초기에 맨 위의 링크가 나오도록 함
                            }
                        ...
                        mobileFrameSetting('.tab-ele', 7); //모바일용
